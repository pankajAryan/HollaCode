...
- (void)viewDidLoad {
    ...
    self.currentSegueIdentifier = HomeControllerId;
...
{
    if ([segue.identifier isEqualToString:HomeControllerId])
    {
        if (self.childViewControllers.count > 0) {
            ...
            ((UIViewController *)segue.destinationViewController).view.frame = CGRectMake(0,parentContainerView.frame.origin.y , self.view.frame.size.width, self.view.frame.size.height);
...
{
    parentContainerView = container_view;
    [self performSegueWithIdentifier:tosegueIdentifer sender:nil];
}
